import { ContactForm } from "@/components/ContactForm";
import { site } from "@/lib/content";

export function Contact() {
  return (
    <section id="contact" className="pt-[120px] pb-40 max-sm:pt-20 max-sm:pb-30">
      <p className="s-tag reveal">Contact</p>
      <div className="s-sep reveal rd1" />

      <div className="reveal rd2 grid grid-cols-2 items-start gap-20 max-lg:grid-cols-1 max-lg:gap-12">
        <div>
          <h2 className="s-title">
            Travaillons <span className="grad">ensemble</span>
          </h2>
          <p
            className="mt-[22px] max-w-[400px] text-[15px]"
            style={{ color: "#303030", lineHeight: 1.85 }}
          >
            Disponible pour des opportunités de stage, des projets freelance ou
            simplement une bonne conversation sur la tech.
          </p>

          <div className="mt-9 flex flex-col gap-4">
            <a
              href={`mailto:${site.email}`}
              className="f-link flex items-center gap-[10px] text-sm"
              style={{ color: "#363636", fontSize: 14 }}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
                <path
                  d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"
                  stroke="hsla(255,55%,75%,.45)"
                  strokeWidth="1.5"
                />
                <polyline
                  points="22,6 12,13 2,6"
                  stroke="hsla(255,55%,75%,.45)"
                  strokeWidth="1.5"
                  fill="none"
                />
              </svg>
              {site.email}
            </a>

            <a
              href={site.github}
              target="_blank"
              rel="noreferrer noopener"
              className="f-link flex items-center gap-[10px]"
              style={{ color: "#363636", fontSize: 14 }}
            >
              <svg
                width="16"
                height="16"
                viewBox="0 0 24 24"
                fill="hsla(255,55%,75%,.45)"
                aria-hidden
              >
                <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
              </svg>
              {site.githubLabel}
            </a>
          </div>
        </div>

        <ContactForm />
      </div>
    </section>
  );
}
